...
export class PartiesScorecardService {
    ...
    if (isDefaultPeriod) {
      ...
      if (precomputed.length > 0) {
        return precomputed
          ...
          .sort((a: any, b: any) => b.mandateConsistencyScore - a.mandateConsistencyScore);
      }
  ...
  async compareScorecards(
    ...
    if (isDefaultPeriod) {
      ...
      if (precomputed.length > 0) {
        ...
        for (const [partyId, rows] of byParty) {
          const party = rows[0].party as any;
          ...
          if (periods.length >= 2) {
            const scores = periods.map((p: any) => p.mandateConsistencyScore);
            if (scores.every((s: any) => s > 0)) {
    ...
    const allPartyIds = Array.from(new Set<string>(partyRows.map((r: any) => r.party_id)));
    const parties = await prisma.party.findMany({
      where: { id: { in: allPartyIds } },
      select: { id: true, abbreviation: true },
    });
    const partyMap = new Map<string, { id: string; abbreviation: string }>(parties.map((p: any) => [p.id, p]));
  ...
  async getCoalitieverwatering(
    ...
    const matches = partyPromises.map((pp: any) => {
    ...
    const survivedCount = matches.filter((m: any) => m.survived).length;
  ...
  async getAvailableYears(): Promise<number[]> {
    ...
    `;
    return result.map((r: any) => r.election_year);
  }
