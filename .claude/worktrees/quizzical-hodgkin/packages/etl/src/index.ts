...
import { runMunicipalSemanticMatching, runAllMunicipalMatching } from './matching/semantic-matcher-municipal.js';
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'semantic-match-municipal': {
        const smmParliament = args.find(a => a === '--parliament') ? args[args.indexOf('--parliament') + 1] : undefined;
        const smmLimit = args.find(a => a === '--limit') ? args[args.indexOf('--limit') + 1] : undefined;
        const smmConcurrency = args.find(a => a === '--concurrency') ? args[args.indexOf('--concurrency') + 1] : undefined;
        const smmDryRun = args.includes('--dry-run');
        const smmResume = args.includes('--resume');
        const smmAll = args.includes('--all');
        initLangfuse();
        try {
          if (smmAll) {
            await runAllMunicipalMatching({
              limit: smmLimit ? parseInt(smmLimit) : undefined,
              dryRun: smmDryRun,
              resume: smmResume,
              concurrency: smmConcurrency ? parseInt(smmConcurrency) : undefined,
            });
          } else if (smmParliament) {
            await runMunicipalSemanticMatching({
              parliament: smmParliament,
              limit: smmLimit ? parseInt(smmLimit) : undefined,
              dryRun: smmDryRun,
              resume: smmResume,
              concurrency: smmConcurrency ? parseInt(smmConcurrency) : undefined,
            ...
            console.log('Usage: npm run ingest semantic-match-municipal --parliament <slug> [--limit N] [--dry-run] [--resume]');
            console.log('       npm run ingest semantic-match-municipal --all');
            console.log('  Parliaments: amsterdam-gemeente, den-haag-gemeente');
            process.exit(1);
          }
        } finally {
          await shutdownLangfuse();
      ...
      default:
        ...
        console.log('');
        console.log('  --- Municipal Semantic Matching (2026) ---');
        console.log('  npm run ingest semantic-match-municipal --parliament amsterdam-gemeente');
        console.log('  npm run ingest semantic-match-municipal --parliament den-haag-gemeente');
        console.log('  npm run ingest semantic-match-municipal --parliament amsterdam-gemeente --resume');
        console.log('  npm run ingest semantic-match-municipal --parliament amsterdam-gemeente --limit 20');
        console.log('  npm run ingest semantic-match-municipal --all');
        console.log('');
